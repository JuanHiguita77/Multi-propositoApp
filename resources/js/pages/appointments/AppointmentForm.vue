<script setup>
	...
	//Volvemos reactivos los campos del formulario
	const form = reactive({
		title: '',
		cliend_id:'',
		start_time: '',
	...
	//evento del formulario para subirse
	...
	{
		//Llevando los datos del formulario a la base de datos
		//tenemos que crear la ruta del endpoint
	...
	const editAppointment = (values, actions) =>
	{
		...
		.then((response) =>
		{
			...
			toastr.success('Appointment Updated succesfully!');
		})
		.catch((error) =>
		{
			actions.setErrors(error.response.data.errors);
	...
	onMounted(()=>
	{
		//Comparar rutas para saber cual mostrar al ahora de editar o crear un appointment
		//Usamos vueRoute
		...
		//configuramos los parametros de flatpickr
		flatpickr(".flatpickr", {
			enableTime: true,
